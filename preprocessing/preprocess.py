...
def preprocess_image(info):
    ...
    if (
        os.path.exists(os.path.join(case_path, "image.npy"))
...
def run(args: Namespace):
    ...
    if args.dataset_type == "AMOS":
        data_splits, modality_info, classes = parse_amos(
            args.dataset_root, args.val_ratio
        )
    elif args.dataset_type == "CHAOS":
        data_splits, modality_info, classes = parse_chaos(
            args.dataset_root, args.test_ratio, args.val_ratio
        )
    elif args.dataset_type == "PROMISE12":
        data_splits, modality_info, classes = parse_promise12(
            args.dataset_root, args.val_ratio
        )
    ...
    output_meta = {
        ...
        "labels": {
            str(idx): label for idx, label in enumerate(["background", *args.classes])
        },
