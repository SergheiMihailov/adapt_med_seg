...
from monai.transforms import LoadImage, EnsureChannelFirst, Compose, ToNumpy
...
chaos_classes = {"1": "Liver", "2": "Right kidney", "3": "Left kidney", "4": "Spleen"}
    ...
    for slc in sorted(os.listdir(path)):
        ...
        if is_ct:
            ...
            for c in chaos_classes.keys():
                ...
                label_img[
                    (label_img >= chaos_cls_intervals[c][0])
                    & (label_img <= chaos_cls_intervals[c][1])
                ] = (float(c) * -1)
...
def parse_chaos(
    data_root: str, test_ratio: float, val_ratio: float
    ...
    for idx in os.listdir(mr_data_root):
        ...
        label_loader_dual = load_callback(
            chaos_label_loader, **{"path": label_path_dual, "is_ct": False}
        )
        # InPhase
        ...
        label_loader_spir = load_callback(
            chaos_label_loader, **{"path": label_path_spir, "is_ct": False}
        )
        # image
        ...
        splits_list.append(
            (
                (f"mr_{idx}_in", image_loader_in, label_loader_dual),
