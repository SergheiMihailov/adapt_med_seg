...
def amos_data_download(data_root: str) -> None:
    ...
    amos_metadata_url = (
        "https://zenodo.org/records/7262581/files/labeled_data_meta_0000_0599.csv"
    )
...
def parse_amos(
    ...
    data_paths = {
        "train": {
            "images": os.path.join(data_root, "amos22", "imagesTr"),
            "labels": os.path.join(data_root, "amos22", "labelsTr"),
        },
        "test": {
            "images": os.path.join(data_root, "amos22", "imagesVa"),
    ...
    for split in ["train", "test"]:
        ...
        for img, lab in zip(images_list, labels_list):
            ...
            if int(img_id) not in metadata:
                ...
                continue
            modality = AMOS_MACHINE_TO_MODALITY[
                metadata[int(img_id)]["Manufacturer's Model Name"]
            ]
            data_splits[split].append(
                (
                    str(img_id),
                    load_callback(amos_image_loader, os.path.join(images_dir, img)),
