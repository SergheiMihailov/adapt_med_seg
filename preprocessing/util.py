...
def three_way_split(*data_lists, test_ratio: float, val_ratio: float):
    ...
    test_indices = np.random.choice(
        len(data_lists[0]), int(test_ratio * len(data_lists[0])), replace=False
    )
    test_indices = set(test_indices)
    train_indices = set(range(len(data_lists[0]))) - test_indices
    val_indices = np.random.choice(
        list(train_indices), int(val_ratio * len(data_lists[0])), replace=False
    )
...
def two_way_split(*data_lists, val_ratio: float):
    ...
    """
    val_indices = np.random.choice(
        len(data_lists[0]), int(val_ratio * len(data_lists[0])), replace=False
    )
