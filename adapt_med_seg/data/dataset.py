...
class MedSegDataset(Dataset):
    ...
    def load_dataset(self) -> None:
        ...
        self._modality_name2id = { # we will likely need this
            v: k for k, v in self._modality_mapping.items()
        }
        ...
        for split in splits:
            ...
            for idx, case_ in enumerate(case_paths):
                self._ct_paths.append(
                    os.path.join(DATASETS_DIR, case_["image"]))
                self._gt_paths.append(
                    os.path.join(DATASETS_DIR, case_["label"]))
                self._case_modality.append(case_["modality"])
