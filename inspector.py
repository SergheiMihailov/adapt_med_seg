...
def load_mask(dir_path):
    ...
    gt_npy = (
        sparse.load_npz(os.path.join(dir_path, gt_npy_path)).toarray().reshape(gt_shape)
    )
    return gt_npy


def load_image_mask(mypath, theirpath: str = None):
    my_image = np.load(os.path.join(mypath, "image.npy")).astype(np.float32)
    my_mask = load_mask(mypath).astype(np.float32)
    if theirpath is not None:
        their_image = np.load(os.path.join(theirpath, "image.npy")).astype(np.float32)
...
def load_all_image_mask(data_idx, mypath, prefix, theirpath):
    ...
    if theirpath is not None:
        their_paths = [
            os.path.join(theirpath, p)
            for p in os.listdir(theirpath)
    ...
    for my_path, their_path in zip(my_paths, their_paths):
        ...
        if their_path is not None:
            ...
            print(
                "average difference in image:", np.mean(np.abs(my_image - their_image))
            )
        ...
        image_mask_list.append(
            (my_name, my_image, my_mask, their_name, their_image, their_mask)
        )
...
def show_image_mask(data_index, data_path, prefix, cls_idx=1):
    ...
    im1 = ax.imshow(
        my_mask[cls_idx, :, :, slice_index].squeeze(),
        cmap="pink",
        alpha=0.4,
        vmin=0,
        vmax=1,
...
def compare_ours_theirs(data_index, data_path, prefix, m3d_path):
    ...
    im02 = ax[0, 2].imshow(
        diff_image[0, :, :, slice_index].squeeze(),
        cmap="gray",
